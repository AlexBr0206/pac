...
public class ProveedorAutenticacion implements AuthenticationProvider{
	...
	public Authentication authenticate (Authentication authentication) throws AuthenticationException{
		...
		String empleado = authentication.getName();
		String clave = authentication.getCredentials().toString();
		HttpServletRequest request =
				((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
		if(servicioUsuario.validar(empleado, clave, request.getSession())) {
			return new UsernamePasswordAuthenticationToken(empleado+clave, clave, new ArrayList<>());
		}
		else {
			throw new
